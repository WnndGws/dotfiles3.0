...
time_now=$(date +%T)
tomorrow=$(date --date="tomorrow" "+%D %H:%m")

mkdir -p /home/wynand/.cache/polybar
file="/home/wynand/.config/polybar_credentials/agenda.txt"
...
if [[ $(date -d $time_now "+%s") -gt $(date -d $time_of_next_event "+%s") ]]; then
    ...
    time_of_next_event_adjusted=$(date -d $time_of_next_event "+%s")
