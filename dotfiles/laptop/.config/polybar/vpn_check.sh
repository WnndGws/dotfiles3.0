...
basedir="/home/wynand/.config/polybar_credentials"

#If was connected, and still is
if [ "$(pgrep openvpn)" ] && [[ -f $basedir/.vpn_connected*.ini ]]; then
    country=$(find $basedir -type f -name "*.vpn_connected*" | cut -d"_" -f4 | rev | cut -d"." -f2 | cut -d"_" -f1 | rev)
    colour="%{u#4E9A06 +u}"
    icon=""
...
#If wasn't connected, still isn't
    ...
    country=$(find $basedir -type f -name "*.vpn_disconnected*" | cut -d"_" -f4 | rev | cut -d"." -f2 | cut -d"_" -f1 | rev)
    colour="%{u#bd2c40 +u}"
    icon=""
fi
